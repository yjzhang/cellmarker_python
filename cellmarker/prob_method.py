...
def prob_test(genes, return_header=False, include_cell_components=False, include_chromosomes=False, params=None,
  ...
  for cell_name in all_cells:
    ...
    genes_pmids_count = set(get_cell_genes_pmids(cell_name, db_dir=db_dir, threshold=params["db_cnt_thre"], use_tfidf=True,
    ...
    cell_prob_vals[cell_name] = [cell_name, -np.inf, overlapping_genes, pmids]
  ...
  for cell_name, prob in res:
    cell_prob_vals[cell_name][1] = prob
    cell_prob_vals[cell_name] = tuple(cell_prob_vals[cell_name])
  ...
  if return_header is True:
    header = ['Cell Name', 'Log-likelihood', 'Overlapping Genes', 'PMIDs']
